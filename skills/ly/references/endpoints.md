...
- **GrpfieldsuniqueValidation 完整参数**(2026-09-07 实测教训):`Fields` 用字段 key(`{"_Type_":"FieldId","Id":"<key>"}`);**`IsCheckAllEntity` 默认 false=只查单据内数据,跨记录唯一必须显式 true**;暂存参与=Checkadata。⚠️ 引用基础资料字段(如 MaterielField materialid)写入元数据成功、读回一致,**但设计器该校验的字段选择器中不可见**——选择器支持的字段类型范围未确认,未经 UI 核验前不要宣称"物料唯一性已生效";验证门槛=元数据读回+设计器选择器双确认。
