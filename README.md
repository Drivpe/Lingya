...
- 需要**已获授权的金蝶环境**(账套 id / 应用 id+密钥 / 用户名),凭证只存本地 `~/.kd/config.json`(token 缓存另存 `~/.ly/`,均不出本机),本仓库零凭据;
...
- [x] v0.3.0 BOTP 写通道与下推闭环(`ly convert-rule new/save` 会话通道写入;自建单据 A→B pushandsave 端到端实测,见 issue #15/#18)
